mutable struct SelfConsistentField
    ...
    function SelfConsistentField(
        ...
        scf_energy    = 0.0
        mo_occ        = get_mo_occ(sys, orbtype=orbtype)::Array{Integer,1}
        ...
        new(
            sys, conv_tol, max_cycle,  # this part are the inputs
            orbtype, iprint,
...
function get_fock(the_scf::SelfConsistentField, rdm1::Array{Float64,2})
    ...
    if the_scf.orbtype == "RHF"
        g = get_jk(the_scf, rdm1)
        fock = (g+the_scf.hcore)::Array{Float64,2}
        return (fock + fock')/2
    elseif the_scf.orbtype == "UHF"
        error("UHF not implemented yet!")
    end
...
function get_rdm1(the_scf::SelfConsistentField, mo_coeff::Array{Float64,2}, mo_occ::Array{Integer,1})
    ...
    for lm=1:the_scf.nao, sg=1:the_scf.nao
        for p = 1:the_scf.nmo
            # @printf("lm = %d, sg = %d, p = %d\n", lm, sg, p)
            # @printf("mo_coeff[p,lm] = % 2.2f\n", mo_coeff[p,lm])
            # @printf("mo_coeff[p,sg] = % 2.2f\n", mo_coeff[p,sg])
            # @printf("mo_occ[p]      = % 2.2f\n", mo_occ[p])
...
function kernel!(the_scf::SelfConsistentField, init_density_guess::Array{Float64,2})
    get_diis_err = (f::Array{Float64,2}, p::Array{Float64,2}) -> norm(reduce(*, [f,p,the_scf.overlp])-reduce(*, [the_scf.overlp,p,f]))
    fock           = get_fock(the_scf, init_density_guess)::Array{Float64,2}
    
    iter       = 0
    diis_err   = get_diis_err(fock, init_density_guess)
